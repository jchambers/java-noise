...
public class Blake2sMessageDigestSpi extends MessageDigestSpi {
  ...
  private void compress(final byte[] bytes, final int offset, final boolean lastBlock) {
    ...
    for (int i = 0; i < 10; i++) {
      final byte[] sigmaRow = SIGMA[i];

      mix(v, 0, 4,  8, 12, messageBlock[sigmaRow[ 0]], messageBlock[sigmaRow[ 1]]);
      mix(v, 1, 5,  9, 13, messageBlock[sigmaRow[ 2]], messageBlock[sigmaRow[ 3]]);
      mix(v, 2, 6, 10, 14, messageBlock[sigmaRow[ 4]], messageBlock[sigmaRow[ 5]]);
      mix(v, 3, 7, 11, 15, messageBlock[sigmaRow[ 6]], messageBlock[sigmaRow[ 7]]);

      mix(v, 0, 5, 10, 15, messageBlock[sigmaRow[ 8]], messageBlock[sigmaRow[ 9]]);
      mix(v, 1, 6, 11, 12, messageBlock[sigmaRow[10]], messageBlock[sigmaRow[11]]);
      mix(v, 2, 7,  8, 13, messageBlock[sigmaRow[12]], messageBlock[sigmaRow[13]]);
      mix(v, 3, 4,  9, 14, messageBlock[sigmaRow[14]], messageBlock[sigmaRow[15]]);
    }
